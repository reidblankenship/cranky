...
void show_version() { println("cranky Version: %s", CRANKY_VERSION); }
...
const std::unordered_map<std::string, size_t> ALL_CMDS_OPTS = {
    std::make_pair("init", 0), std::make_pair("version", 0),
    std::make_pair("help", 0), std::make_pair("help", 1),
    std::make_pair("new", 1),
};
...
cranky_vec_args cmd_vec(int argc, char **argv) {
  ...
  const auto &cmd_one_option =
      (CMD_ONE_OPT.contains(FIRST_ARG) && (SECOND_ARG != nullptr) && argc == 3);

  if (cmd_no_options) {
    ret.emplace_back(FIRST_ARG, std::nullopt);
  } else if (cmd_one_option) {
    ret.emplace_back(FIRST_ARG, std::make_optional<std::string>(SECOND_ARG));
  } else {
...
void Command::print_args(const cranky_vec_args &args) const {
  ...
  for (auto cmd = args.begin(); cmd != args.end(); cmd++) {
    println("cmd[%zu].first = %s, .second = %s", i, cmd->first.c_str(),
